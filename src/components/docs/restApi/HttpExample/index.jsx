...
function addSearchParamsToRequestUrl(url, link) {
  ...
  for (const [param, value] of Object.entries(schemaExampleFor(link.hrefSchema))) {
...
function buildFetchRequest({ request }, link) {
  const rawRequestUrl = (request?.url || link.href).replace(regexp, (_matched, chunk) => {
    if (chunk === 'item_type') {
...
async function foo() {
  const jobResultSelfEndpoint = await findJobResultSelfEndoint;
    ...
    return (
      <>
        {examples.map((example) => (
          <HttpExample key={example.id} example={example} link={link} startExpanded={true} />
        ))}
      </>
    );
  }
  ...
  return <HttpExample example={{ title: 'Basic example' }} link={link} startExpanded={true} />;
}
