...
export default function GraphQlDemo({ height, children }) {
  ...
  const difference =
    text.slice(0, cursor).length - text.slice(0, cursor).replace(/ {2}/g, '\t').length;

  return (
    <div className={s.root}>
      <div className={s.code} style={{ height: `calc(${1.3 * height}em + var(--padding) * 2)` }}>
        {text
          .replace(/ {2}/g, '\t')
      ...
      <Highlight {...defaultProps} code={result.replace(/ {2}/g, '\t')} language="json">
