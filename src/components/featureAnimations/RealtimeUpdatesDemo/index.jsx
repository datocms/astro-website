...
export default function RealtimeUpdatesDemo() {
  ...
  const next = useCallback(() => {
    setPost((post) => (post >= images.length ? post - images.length + 1 : post + 1));
  }, [setPost]);
          ...
          <TransitionGroup className={s.page}>
            ...
            <CSSTransition key="static-2" timeout={100}>
              <div className={s.pageSubtitle}>
                <div className={s.ping} /> Connected to DatoCMS, receiving live updates!
              </div>
            </CSSTransition>
            {range(post + 3, post, -1).map((i) => {
              const indexInRange =
                i < 0 ? i + images.length : i >= images.length ? i - images.length : i;
